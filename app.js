...
function saveActive() {
  if (!db) return;
  db.transaction("active", "readwrite").objectStore("active").put({
    id: "current",
    startedAt: startTime,
...
function recoverActiveTrack() {
  if (!db) return;
  db.transaction("active", "readonly").objectStore("active").get("current").onsuccess = e => {
    const saved = e.target.result;
    if (!saved || !saved.points || saved.points.length < 2) return;

    if (confirm(`An unfinished track was found (${formatDistance(saved.distance)}). Resume it?`)) {
...
function showPosition(position, center = false) {
  const p = {
  ...
  if (center) map.setView([p.lat, p.lon], 15, { animate: false });
  return p;
}
...
function initialLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    error => {
      const messages = {1: "Location permission denied", 2: "Location unavailable", 3: "GPS timeout"};
      setStatus(messages[error.code] || "GPS unavailable");
    },
...
function receivePosition(position) {
  ...
  const p = showPosition(position, false);
  if (!p) return;
...
function loadTracks() {
  if (!db) return;
  db.transaction("tracks", "readonly").objectStore("tracks").getAll().onsuccess = e => {
    ...
    for (const track of tracks) {
      ...
      card.innerHTML = `
        <div class="date">${new Date(track.date).toLocaleString()}</div>
...
function showTrack(track) {
  ...
  const duration = Math.max(0.001, ((track.endedAt || Date.now()) - track.startedAt) / 1000);
  $("averageSpeed").textContent = `${(track.distance / duration * 3.6).toFixed(1)} km/h`;
}
...
dbRequest.onerror = () => setStatus("Database error");
